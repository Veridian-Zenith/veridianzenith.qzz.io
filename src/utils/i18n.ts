// veridianzenith.qzz.io/src/utils/i18n.ts
import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';
import LanguageDetector from 'i18next-browser-languagedetector';

const resources = {
  en: {
    translation: {
      "nav.home": "Home",
      "nav.about": "About",
      "nav.projects": "Projects",
      "nav.aur": "AUR",
      "nav.contact": "Contact",
      "hero.title": "Veridian Zenith",
      "hero.subtitle": "Forging digital realms where mystical aesthetics meet cutting-edge performance.",
      "hero.void": "Step into the Nordic void.",
      "hero.explore": "Explore Artifacts",
      "hero.summon": "Summon",
      "hero.architect": "The Architect",
      "footer.sigil": "Sigil",
      "footer.copyright": "© {{year}} Veridian Zenith",

      "about.title": "The Nordic Zenith",
      "about.subtitle": "\"Where ancient craftsmanship meets digital excellence.\"",
      "about.forge.title": "Our Digital Forge",
      "about.forge.description": "Veridian Zenith is more than a collective—it is a digital forge. We operate in the intersection of ancient aesthetics and cutting-edge performance. Our focus lies in building systems that are as resilient as they are beautiful.",
      "about.philosophy.title": "The Philosophy",
      "about.philosophy.description": "Every line of code is intentional. Like runes carved into stone, our software is built for permanence, clarity, and uncompromising speed. We believe in the \"Nordic Way\"—minimalist, effective, and powerful.",
      "about.license.title": "The License",
      "about.license.description": "We believe in the sanctity of open source. Veridian Zenith artifacts are released under the ",
      "about.license.link": "Open Software License 3.0",
      "about.license.description2": ", ensuring the code remains free and accessible to all who seek it.",

      "projects.title": "Arcane Artifacts",
      "projects.subtitle": "Explore the digital runes we've carved. Influence the forge by casting your vote on the projects that matter most to you.",
      "projects.inspect": "Inspect Rune",
      "projects.influence": "Influence Forge",
      "projects.future": "Future runes are being etched...",
      "projects.vote.title": "Record Your Intent",
      "projects.vote.subtitle": "Provide your details to cast your vote for",
      "projects.vote.name": "Name",
      "projects.vote.name_placeholder": "Arbiter Name",
      "projects.vote.email": "Email",
      "projects.vote.email_placeholder": "void@zenith.com",
      "projects.vote.discard": "Discard",
      "projects.vote.cast": "Cast Rune",
      "projects.axiomos.description": "A custom 64-bit operating system built from scratch using Zig. Targets UEFI/Secure Boot with a focus on modern Intel hardware and high-performance kernel architecture.",
      "projects.voix.description": "A secure privilege management tool (sudo/doas alternative) featuring PAM authentication, Lua-based configuration, and a focus on minimal attack surface.",
      "projects.meshiji.description": "A modern, Linux-only file explorer built with Flutter. Provides a clean, intuitive interface for managing files.",
      "projects.peguni.description": "A text-based virtual pet simulator featuring a unique constructed language (Vaesktöng). A companion in the terminal, built for the void.",
      "projects.misc.description": "A repository of experimental artifacts and system utilities, including ZigSysMon—a lightweight /proc monitor—alongside benchmarks and FFI experiments.",

      "aur.title": "AUR Repositories",
      "aur.subtitle": "Pre-compiled artifacts for Arch Linux, ready for direct integration.",
      "aur.maintainer": "Maintainer",
      "aur.install.title": "Installation Guide",
      "aur.install.comment": "# Install with yay",
      "aur.packages.veridian-zenith-git.description": "The official Veridian Zenith theme package for Arch Linux systems.",
      "aur.packages.veridian-icons-git.description": "Icon pack complementing the Veridian Zenith aesthetic.",
      "aur.packages.veridian-cursors.description": "Cursor theme for the complete Nordic void experience.",

      "contact.title": "Summon The Architect",
      "contact.subtitle": "\"The runes are cast, the signals sent. Reach through the void and influence the digital forge.\"",
      "contact.invocation.title": "Direct Invocation",
      "contact.invocation.description": "Whether for collaboration, consultation, or to report a fracture in reality, the Architect awaits.",
      "contact.architect.label": "Architect",
      "contact.architect.sub": "Lead Developer",
      "contact.email.label": "Email",
      "contact.forge.label": "The Forge",
      "contact.connect": "Connect",
      "contact.community.title": "Zenith Commons",
      "contact.community.description": "Step into the collective void. Join our community for real-time discourse and project updates.",
      "contact.community.join": "ENTER THE DISCORD",
    }
  },
  ru: {
    translation: {
      "nav.home": "Главная",
      "nav.about": "О нас",
      "nav.projects": "Проекты",
      "nav.aur": "AUR",
      "nav.contact": "Контакты",
      "hero.title": "Veridian Zenith",
      "hero.subtitle": "Создание цифровых миров, где мистическая эстетика встречается с передовой производительностью.",
      "hero.void": "Шагните в нордическую пустоту.",
      "hero.explore": "Исследовать артефакты",
      "hero.summon": "Призвать",
      "hero.architect": "Архитектора",
      "footer.sigil": "Сигил",
      "footer.copyright": "© {{year}} Veridian Zenith",

      "about.title": "Нордический Зенит",
      "about.subtitle": "\"Где древнее мастерство встречается с цифровым совершенством.\"",
      "about.forge.title": "Наша цифровая кузница",
      "about.forge.description": "Veridian Zenith - это больше, чем коллектив, это цифровая кузница. Мы работаем на стыке древней эстетики и передовых технологий. Наша цель - создавать системы, которые настолько же устойчивы, насколько и красивы.",
      "about.philosophy.title": "Философия",
      "about.philosophy.description": "Каждая строка кода имеет значение. Подобно рунам, высеченным на камне, наше программное обеспечение создано для долговечности, ясности и бескомпромиссной скорости. Мы верим в \"нордический путь\" - минималистичный, эффективный и мощный.",
      "about.license.title": "Лицензия",
      "about.license.description": "Мы верим в святость открытого исходного кода. Артефакты Veridian Zenith выпускаются под ",
      "about.license.link": "Open Software License 3.0",
      "about.license.description2": ", гарантируя, что код остается бесплатным и доступным для всех, кто его ищет.",

      "projects.title": "Тайные артефакты",
      "projects.subtitle": "Исследуйте высеченные нами цифровые руны. Влияйте на кузницу, голосуя за самые важные для вас проекты.",
      "projects.inspect": "Осмотреть руну",
      "projects.influence": "Повлиять на кузницу",
      "projects.future": "Будущие руны уже высекаются...",
      "projects.vote.title": "Зафиксируйте свое намерение",
      "projects.vote.subtitle": "Предоставьте свои данные, чтобы проголосовать за",
      "projects.vote.name": "Имя",
      "projects.vote.name_placeholder": "Имя арбитра",
      "projects.vote.email": "Электронная почта",
      "projects.vote.email_placeholder": "void@zenith.com",
      "projects.vote.discard": "Отказаться",
      "projects.vote.cast": "Бросить руну",
      "projects.axiomos.description": "Пользовательская 64-битная операционная система, созданная с нуля с использованием Zig. Предназначена для UEFI/Secure Boot с акцентом на современное оборудование Intel и высокопроизводительную архитектуру ядра.",
      "projects.voix.description": "Безопасный инструмент управления привилегиями (альтернатива sudo/doas) с аутентификацией PAM, конфигурацией на основе Lua и минимальной поверхностью атаки.",
      "projects.meshiji.description": "Современный файловый менеджер только для Linux, созданный с помощью Flutter. Обеспечивает чистый, интуитивно понятный интерфейс для управления файлами.",
      "projects.peguni.description": "Текстовый симулятор виртуального питомца с уникальным искусственным языком (Vaesktöng). Компаньон в терминале, созданный для пустоты.",
      "projects.misc.description": "Репозиторий экспериментальных артефактов и системных утилит, включая ZigSysMon — легковесный монитор /proc — а также тесты производительности и эксперименты с FFI.",

      "aur.title": "Репозитории AUR",
      "aur.subtitle": "Предварительно скомпилированные артефакты для Arch Linux, готовые к прямой интеграции.",
      "aur.maintainer": "Сопровождающий",
      "aur.install.title": "Руководство по установке",
      "aur.install.comment": "# Установить с помощью yay",
      "aur.packages.veridian-zenith-git.description": "Официальный пакет темы Veridian Zenith для систем Arch Linux.",
      "aur.packages.veridian-icons-git.description": "Пакет иконок, дополняющий эстетику Veridian Zenith.",
      "aur.packages.veridian-cursors.description": "Тема курсора для полного погружения в нордическую пустоту.",

      "contact.title": "Призвать Архитектора",
      "contact.subtitle": "\"Руны брошены, сигналы отправлены. Протяните руку через пустоту и повлияйте на цифровую кузницу.\"",
      "contact.invocation.title": "Прямой вызов",
      "contact.invocation.description": "Будь то для сотрудничества, консультации или сообщения о разломе в реальности, Архитектор ждет.",
      "contact.architect.label": "Архитектор",
      "contact.architect.sub": "Ведущий разработчик",
      "contact.email.label": "Электронная почта",
      "contact.forge.label": "Кузница",
      "contact.connect": "Подключиться",
      "contact.community.title": "Община Зенита",
      "contact.community.description": "Шагните в коллективную пустоту. Присоединяйтесь к нашему сообществу для обсуждений в реальном времени и получения обновлений по проектам.",
      "contact.community.join": "ВОЙТИ В DISCORD",
    }
  },
  de: {
    translation: {
      "nav.home": "Startseite",
      "nav.about": "Über uns",
      "nav.projects": "Projekte",
      "nav.aur": "AUR",
      "nav.contact": "Kontakt",
      "hero.title": "Veridian Zenith",
      "hero.subtitle": "Schmieden digitaler Reiche, in denen mystische Ästhetik auf Spitzenleistung trifft.",
      "hero.void": "Treten Sie ein in die nordische Leere.",
      "hero.explore": "Artefakte erkunden",
      "hero.summon": "Beschwöre",
      "hero.architect": "den Architekten",
      "footer.sigil": "Siegel",
      "footer.copyright": "© {{year}} Veridian Zenith",

      "about.title": "Der Nordische Zenit",
      "about.subtitle": "\"Wo alte Handwerkskunst auf digitale Exzellenz trifft.\"",
      "about.forge.title": "Unsere digitale Schmiede",
      "about.forge.description": "Veridian Zenith ist mehr als ein Kollektiv - es ist eine digitale Schmiede. Wir agieren an der Schnittstelle von alter Ästhetik und modernster Leistung. Unser Fokus liegt auf der Entwicklung von Systemen, die ebenso widerstandsfähig wie schön sind.",
      "about.philosophy.title": "Die Philosophie",
      "about.philosophy.description": "Jede Codezeile ist beabsichtigt. Wie in Stein gemeißelte Runen ist unsere Software auf Beständigkeit, Klarheit und kompromisslose Geschwindigkeit ausgelegt. Wir glauben an den \"nordischen Weg\" - minimalistisch, effektiv und leistungsstark.",
      "about.license.title": "Die Lizenz",
      "about.license.description": "Wir glauben an die Heiligkeit von Open Source. Die Artefakte von Veridian Zenith werden unter der ",
      "about.license.link": "Open Software License 3.0",
      "about.license.description2": " veröffentlicht, um sicherzustellen, dass der Code für alle, die ihn suchen, frei und zugänglich bleibt.",

      "projects.title": "Arkane Artefakte",
      "projects.subtitle": "Erkunden Sie die von uns geschaffenen digitalen Runen. Beeinflussen Sie die Schmiede, indem Sie für die Projekte stimmen, die Ihnen am wichtigsten sind.",
      "projects.inspect": "Rune inspizieren",
      "projects.influence": "Schmiede beeinflussen",
      "projects.future": "Zukünftige Runen werden gerade geschmiedet...",
      "projects.vote.title": "Ihre Absicht festhalten",
      "projects.vote.subtitle": "Geben Sie Ihre Daten an, um für",
      "projects.vote.name": "Name",
      "projects.vote.name_placeholder": "Name des Schiedsrichters",
      "projects.vote.email": "E-Mail",
      "projects.vote.email_placeholder": "void@zenith.com",
      "projects.vote.discard": "Verwerfen",
      "projects.vote.cast": "Rune werfen",
      "projects.axiomos.description": "Ein benutzerdefiniertes 64-Bit-Betriebssystem, das von Grund auf mit Zig entwickelt wurde. Zielt auf UEFI/Secure Boot ab, mit einem Fokus auf moderner Intel-Hardware und einer hochleistungsfähigen Kernel-Architektur.",
      "projects.voix.description": "Ein sicheres Werkzeug zur Rechteverwaltung (Alternative zu sudo/doas) mit PAM-Authentifizierung, Lua-basierter Konfiguration und minimaler Angriffsfläche.",
      "projects.meshiji.description": "Ein moderner, nur für Linux verfügbarer Dateiexplorer, der mit Flutter entwickelt wurde. Bietet eine saubere, intuitive Oberfläche zur Verwaltung von Dateien.",
      "projects.peguni.description": "Ein textbasierter virtueller Haustiersimulator mit einer einzigartigen konstruierten Sprache (Vaesktöng). Ein Begleiter im Terminal, geschaffen für die Leere.",
      "projects.misc.description": "Ein Repository mit experimentellen Artefakten und System-Utilities, einschließlich ZigSysMon – einem leichtgewichtigen /proc-Monitor – sowie Benchmarks und FFI-Experimenten.",

      "aur.title": "AUR-Repositorien",
      "aur.subtitle": "Vorkompilierte Artefakte für Arch Linux, bereit zur direkten Integration.",
      "aur.maintainer": "Betreuer",
      "aur.install.title": "Installationsanleitung",
      "aur.install.comment": "# Mit yay installieren",
      "aur.packages.veridian-zenith-git.description": "Das offizielle Veridian Zenith Theme-Paket für Arch Linux-Systeme.",
      "aur.packages.veridian-icons-git.description": "Icon-Paket, das die Ästhetik von Veridian Zenith ergänzt.",
      "aur.packages.veridian-cursors.description": "Cursor-Theme für das vollständige Erlebnis der nordischen Leere.",

      "contact.title": "Den Architekten beschwören",
      "contact.subtitle": "\"Die Runen sind geworfen, die Signale gesendet. Greifen Sie durch die Leere und beeinflussen Sie die digitale Schmiede.\"",
      "contact.invocation.title": "Direkte Anrufung",
      "contact.invocation.description": "Ob für Zusammenarbeit, Beratung oder um einen Bruch in der Realität zu melden, der Architekt wartet.",
      "contact.architect.label": "Architekt",
      "contact.architect.sub": "Hauptentwickler",
      "contact.email.label": "E-Mail",
      "contact.forge.label": "Die Schmiede",
      "contact.connect": "Verbinden",
      "contact.community.title": "Zenith-Gemeinschaft",
      "contact.community.description": "Treten Sie in die kollektive Leere ein. Treten Sie unserer Gemeinschaft für Echtzeit-Diskussionen und Projekt-Updates bei.",
      "contact.community.join": "DISCORD BEITRETEN",
    }
  },
  ko: {
    translation: {
      "nav.home": "홈",
      "nav.about": "소개",
      "nav.projects": "프로젝트",
      "nav.aur": "AUR",
      "nav.contact": "연락처",
      "hero.title": "Veridian Zenith",
      "hero.subtitle": "신비로운 미학이 최첨단 성능과 만나는 디지털 영역을 구축합니다.",
      "hero.void": "북유럽의 공허 속으로.",
      "hero.explore": "유물 탐험",
      "hero.summon": "소환",
      "hero.architect": "건축가",
      "footer.sigil": "인장",
      "footer.copyright": "© {{year}} Veridian Zenith",

      "about.title": "북유럽의 제니스",
      "about.subtitle": "\"고대 장인 정신이 디지털 우수성과 만나는 곳.\"",
      "about.forge.title": "우리의 디지털 대장간",
      "about.forge.description": "Veridian Zenith는 단순한 집단이 아니라 디지털 대장간입니다. 우리는 고대 미학과 최첨단 성능이 교차하는 지점에서 활동합니다. 우리의 초점은 아름다움만큼이나 탄력적인 시스템을 구축하는 데 있습니다.",
      "about.philosophy.title": "철학",
      "about.philosophy.description": "모든 코드 라인은 의도적입니다. 돌에 새겨진 룬 문자처럼 우리의 소프트웨어는 영속성, 명확성 및 타협하지 않는 속도를 위해 제작되었습니다. 우리는 '북유럽 방식'인 미니멀리스트, 효과적, 강력함을 믿습니다.",
      "about.license.title": "라이선스",
      "about.license.description": "우리는 오픈 소스의 신성함을 믿습니다. Veridian Zenith의 결과물은 ",
      "about.license.link": "Open Software License 3.0",
      "about.license.description2": "에 따라 배포되어 코드를 찾는 모든 사람이 자유롭게 접근하고 사용할 수 있도록 보장합니다.",

      "projects.title": "비전 유물",
      "projects.subtitle": "우리가 조각한 디지털 룬을 탐험하세요. 가장 중요한 프로젝트에 투표하여 대장간에 영향을 미치세요.",
      "projects.inspect": "룬 검사",
      "projects.influence": "대장간에 영향",
      "projects.future": "미래의 룬이 새겨지고 있습니다...",
      "projects.vote.title": "의도 기록",
      "projects.vote.subtitle": "투표하려면 세부 정보를 제공하세요",
      "projects.vote.name": "이름",
      "projects.vote.name_placeholder": "중재자 이름",
      "projects.vote.email": "이메일",
      "projects.vote.email_placeholder": "void@zenith.com",
      "projects.vote.discard": "취소",
      "projects.vote.cast": "룬 던지기",
      "projects.axiomos.description": "Zig를 사용하여 처음부터 구축된 맞춤형 64비트 운영 체제. 최신 인텔 하드웨어 및 고성능 커널 아키텍처에 중점을 둔 UEFI/Secure Boot를 대상으로 합니다.",
      "projects.voix.description": "PAM 인증, Lua 기반 구성 및 최소한의 공격 표면에 중점을 둔 안전한 권한 관리 도구(sudo/doas 대안).",
      "projects.meshiji.description": "Flutter로 제작된 현대적인 Linux 전용 파일 탐색기. 파일을 관리하기 위한 깨끗하고 직관적인 인터페이스를 제공합니다.",
      "projects.peguni.description": "독특한 인공 언어(Vaesktöng)를 특징으로 하는 텍스트 기반 가상 애완 동물 시뮬레이터. 공허를 위해 만들어진 터미널의 동반자입니다.",
      "projects.misc.description": "경량 /proc 모니터인 ZigSysMon을 포함한 실험적인 아티팩트 및 시스템 유틸리티 저장소이며, 벤치마크 및 FFI 실험도 포함합니다.",

      "aur.title": "AUR 저장소",
      "aur.subtitle": "Arch Linux용으로 사전 컴파일된 아티팩트로, 직접 통합할 준비가 되어 있습니다.",
      "aur.maintainer": "관리자",
      "aur.install.title": "설치 안내서",
      "aur.install.comment": "# yay로 설치",
      "aur.packages.veridian-zenith-git.description": "Arch Linux 시스템용 공식 Veridian Zenith 테마 패키지입니다.",
      "aur.packages.veridian-icons-git.description": "Veridian Zenith 미학을 보완하는 아이콘 팩입니다.",
      "aur.packages.veridian-cursors.description": "완벽한 북유럽 공허 경험을 위한 커서 테마입니다.",

      "contact.title": "건축가 소환",
      "contact.subtitle": "\"룬이 던져지고 신호가 보내졌습니다. 공허를 통해 손을 뻗어 디지털 대장간에 영향을 미치세요.\"",
      "contact.invocation.title": "직접 호출",
      "contact.invocation.description": "협업, 상담 또는 현실의 균열을 보고하기 위해 건축가가 기다리고 있습니다.",
      "contact.architect.label": "건축가",
      "contact.architect.sub": "수석 개발자",
      "contact.email.label": "이메일",
      "contact.forge.label": "대장간",
      "contact.connect": "연결",
      "contact.community.title": "제니스 커먼즈",
      "contact.community.description": "집단적 공허 속으로 들어가십시오. 실시간 담론 및 프로젝트 업데이트를 위해 커뮤니티에 가입하십시오.",
      "contact.community.join": "디스코드 입장",
    }
  }
};

i18n
  .use(LanguageDetector)
  .use(initReactI18next)
  .init({
    resources,
    fallbackLng: 'en',
    interpolation: {
      escapeValue: false
    }
  });

export default i18n;
